"""Discussion Cog for the "BotAToutFer" discord bot
...
class Discussion(commands.Cog):
    ...
    async def ptdrtki(self, ctx):
        response = (
            ...
            f"Alors pour ta culture je suis le grand {self.bot.user.mention}, "
            "le bot Discord qui fait tout, même le café ! "
        )
    ...
    async def noel(self, ctx):
        ...
        noel = datetime.datetime(now.year, 12, 25, 0, 0, 0)
        time_delta = noel - now
        if time_delta.days < 0:
            time_delta = datetime.datetime(now.year + 1, 12, 25, 0, 0, 0) - now
        ...
        response = f"Il reste {days} jour(s), {hours} heure(s), {minutes} " \
                   f"minute(s) et {seconds} seconde(s) avant Noël ! " \
                   ":christmas_tree: "

        await ctx.send(response)

        response = f"Un peu de patience {ctx.author.mention}, je sais que " \
                   "tu veux tes cadeaux petit vénal :gift:"
        await ctx.send(response)
    ...
    async def crepes(self, ctx):
        response = (
            ...
            f"- {random.randint(300, 500)} g de beurre fondu\n"
            "1/ Dans un grand récipient, mélangez les œufs, le lait ainsi "
            "que la farine.\n "
            ...
            "7/ Huilez une crêpière très chaude et formez des crêpes à "
            "l'aide d'une petite louche.\n "
