...
class Pictures(commands.Cog):
    ...
    async def cat(self):
        for channel in self.bot.autorized_channels:
            if random.randrange(200) < 1 and datetime.datetime.now().hour in range(7, 23): # Statisticly send 1 message per day (one chance on 160 every 6 minutes between 7AM and 11PM)
                try:
