...
class Anniversaire(commands.Cog):
    ...
    async def anniv(self, ctx, user: discord.User):
        if user:
            try:
                ...
                self.bot.log.error("birthdays.json file not found: ", exc_info=e)
                birthdays = {}
    ...
    @commands.command(name="mon-anniv", help="Permet d'enregistrer sa date de naissance sous la forme DD/MM/YYYY")
